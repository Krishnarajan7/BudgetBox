...
///
/// `GET /v1/changes?since=` names the rows that moved; the list endpoints
/// hand over the rows themselves. Everything is matched back to a local row
...
class SyncPuller {
  ...
  /// The watermark, kept in the Settings table the rest of the box already
  /// uses for its own facts.
  static const sinceKey = 'sync.since';

  /// A txn per request; a month of catching up is still one screenful.
  static const _txnFetchCap = 300;
  ...
  Future<DateTime> watermark() async {
    final row = await (_db.select(_db.settings)
          ..where((s) => s.key.equals(sinceKey)))
        .getSingleOrNull();
    final parsed = row == null ? null : DateTime.tryParse(row.value);
    return parsed?.toUtc() ?? DateTime.utc(1970);
  }

  Future<void> _setWatermark(DateTime at) {
    return _db.into(_db.settings).insertOnConflictUpdate(
          SettingsCompanion(
            key: const Value(sinceKey),
            value: Value(at.toUtc().toIso8601String()),
          ),
        );
  }
  ...
  Future<PullOutcome> pull(SyncWire wire, {required bool queueEmpty}) async {
    ...
    final since = await watermark();

    final ({DateTime now, Map<String, List<String>> changed}) feed;
    try {
      feed = await wire.changes(since);
    } on BbxOffline catch (e) {
      return PullOutcome(applied: 0, offline: true, detail: e.detail);
    }

    final changed = Map<String, List<String>>.from(feed.changed);
    // Balances are derived upstream from anchors plus txn deltas, so a txn
    // moving means the accounts moved even though no account row was touched.
    if (changed.containsKey('txns') || changed.containsKey('balance_anchors')) {
      changed.putIfAbsent('accounts', () => const []);
    }
    ...
    try {
      ...
      // Half a pull is fine; the watermark stays put so the rest arrives next
      // time rather than being skipped.
      return PullOutcome(applied: applied, offline: true, detail: e.detail);
    }

    await _setWatermark(feed.now);
    return PullOutcome(applied: applied, offline: false, at: feed.now);
  }
  ...
  static Future<int> _applyAccounts(
      SyncPuller p, SyncWire wire, List<String> ids) async {
    ...
    for (final r in rows) {
      n += await p._absorb(SyncKinds.account, '${r['id']}', (localId) async {
        ...
        if (localId == null) {
          return p._db.into(p._db.accounts).insert(
        ...
        await (p._db.update(p._db.accounts)
          ...
          balancePaise: p._trustDerived ? Value(balance) : const Value.absent(),
  ...
  static Future<int> _applyCategories(
    ...
    for (final r in rows) {
      n += await p._absorb(SyncKinds.category, '${r['id']}', (localId) async {
        ...
        final kind = _pick(CategoryKind.values, r['kind'], CategoryKind.expense);
        final sortOrder = _int(r['sort_order']);
        final archived = r['archived'] == true;
        if (localId == null) {
          return p._db.into(p._db.categories).insert(
        ...
        await (p._db.update(p._db.categories)
  ...
  static Future<int> _applyGoals(
    ...
    for (final view in views) {
      ...
      n += await p._absorb(SyncKinds.goal, '${r['id']}', (localId) async {
        ...
        if (localId == null) {
          return p._db.into(p._db.goals).insert(
        ...
        await (p._db.update(p._db.goals)..where((g) => g.id.equals(localId)))
  ...
  static Future<int> _applyRecurrings(
    ...
    for (final r in rows) {
      final accountId =
          await p._ids.localFor(SyncKinds.account, '${r['account_id']}');
      ...
      n += await p._absorb(SyncKinds.recurring, '${r['id']}', (localId) async {
        final companion = RecurringsCompanion(
          ...
          kind: Value(_pick(RecurringKind.values, r['kind'],
        ...
        await (p._db.update(p._db.recurrings)
              ..where((x) => x.id.equals(localId)))
            .write(companion);
        return localId;
      });
    }
    ...
    for (final r in rows) {
      ...
      n += await p._absorb(SyncKinds.budget, '${r['id']}', (localId) async {
        final companion = BudgetsCompanion(
          ...
          period: Value(_pick(
              BudgetPeriod.values, r['period'], BudgetPeriod.month)),
        ...
        await (p._db.update(p._db.budgets)..where((b) => b.id.equals(localId)))
            .write(companion);
        return localId;
      });
    }
  ...
  static Future<int> _applyPinned(
    ...
    for (final r in rows) {
      ...
      n += await p._absorb(SyncKinds.pinned, '${r['id']}', (localId) async {
        ...
        await (p._db.update(p._db.pinneds)..where((x) => x.id.equals(localId)))
            .write(companion);
        return localId;
      });
    }
  ...
  static Future<int> _applyTxns(
      SyncPuller p, SyncWire wire, List<String> ids) async {
    var n = 0;
    for (final remoteId in ids.take(_txnFetchCap)) {
      ...
      n += await p._absorb(SyncKinds.txn, '${row['id']}', (localId) async {
        ...
        await (p._db.update(p._db.txns)..where((t) => t.id.equals(localId)))
            .write(companion);
        return localId;
      });
    }
  ...
  static Future<int> _applySeals(
      SyncPuller p, SyncWire wire, List<String> days) async {
    ...
    for (final r in rows) {
      ...
      n += await p._absorb(SyncKinds.seal, day, (_) async {
        await p._db.into(p._db.daySeals).insertOnConflictUpdate(
              DaySealsCompanion(
                date: Value(day),
  ...
  static Future<int> _applyJournal(
    ...
    for (final r in rows) {
      ...
      n += await p._absorb(SyncKinds.journal, day, (_) async {
        await p._db.into(p._db.journalEntries).insertOnConflictUpdate(
  ...
  static Future<int> _applyNotes(
    ...
    for (final r in rows) {
      n += await p._absorb(SyncKinds.note, '${r['id']}', (localId) async {
        ...
        await (p._db.update(p._db.notes)..where((x) => x.id.equals(localId)))
            .write(companion);
        return localId;
      });
    }
  ...
  static Future<int> _applyFocus(
    ...
    for (final r in rows) {
      n += await p._absorb(SyncKinds.focus, '${r['id']}', (localId) async {
        ...
        await (p._db.update(p._db.focusSessions)
              ..where((x) => x.id.equals(localId)))
            .write(companion);
        return localId;
      });
    ...
    for (final r in rows) {
      n += await p._absorb(SyncKinds.event, '${r['id']}', (localId) async {
        final companion = EventsCompanion(
          ...
          repeat:
              Value(_pick(EventRepeat.values, r['repeat'], EventRepeat.none)),
          archived: Value(r['archived'] == true),
        );
        if (localId == null) {
          return p._db.into(p._db.events).insert(companion);
        }
        await (p._db.update(p._db.events)..where((x) => x.id.equals(localId)))
            .write(companion);
        return localId;
      });
    }
  ...
  static Future<int> _applyVault(
    ...
    for (final r in rows) {
      n += await p._absorb(SyncKinds.vault, '${r['id']}', (localId) async {
        ...
        await (p._db.update(p._db.vaultItems)
              ..where((x) => x.id.equals(localId)))
            .write(companion);
        return localId;
      });
  ...
  static Map<String, dynamic>? _span(List<String> days) {
    ...
    return {'from_day': valid.first, 'to_day': valid.last};
  }
