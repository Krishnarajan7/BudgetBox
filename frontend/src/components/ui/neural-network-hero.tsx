...
export default function Hero({
  ...
  microDetails = ["Tasks", "Habits", "Expenses", "Sleep"]
}: HeroProps) {
  ...
  const microItem4Ref = useRef<HTMLLIElement | null>(null);
  ...
  useGSAP(
    () => {
      ...
      document.fonts.ready.then(() => {
        ...
        const microItems = [microItem1Ref.current, microItem2Ref.current, microItem3Ref.current, microItem4Ref.current].filter(Boolean);
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
        ...
        <ul ref={microRef} className="mt-8 flex flex-wrap gap-6 text-xs font-extralight tracking-tight text-white/60">
          {microDetails.map((detail, index) => {
            const refMap = [microItem1Ref, microItem2Ref, microItem3Ref, microItem4Ref];
            return (
