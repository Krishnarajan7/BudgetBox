...
import { Droplets, Plus, Minus, Target, TrendingUp, Trash2, Pencil, Loader2, AlertCircle } from "lucide-react";
...
export default function Water() {
  ...
  const [tempGoal, setTempGoal] = useState(DEFAULT_WATER_GOAL.toString());
  ...
  const {
    ...
    queryKey: ["water"],
    queryFn: () => listWaterLogs(),
  });

  const todayEntry = history.find(e => e.date === todayStr);
  const glasses = todayEntry?.glasses ?? 0;
  const goal = todayEntry?.goal ?? DEFAULT_WATER_GOAL;

  const upsertMutation = useMutation({
    mutationFn: (vars: { date: string; glasses: number; goal?: number }) =>
      upsertWaterLog(vars.date, { glasses: vars.glasses, goal: vars.goal }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["water"] });
    },
    onError: () => {
      toast({
        title: "Couldn't save water log",
  ...
  const deleteMutation = useMutation({
    mutationFn: (date: string) => deleteWaterLog(date),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["water"] });
    },
    onError: () => {
  ...
  const updateToday = (newGlasses: number) => {
    ...
    upsertMutation.mutate({ date: todayStr, glasses: value });
  };

  const updateEntry = () => {
    if (!editingEntry) return;
    upsertMutation.mutate(
      { date: editingEntry.date, glasses: editGlasses },
  ...
  const updateGoal = () => {
    const newGoal = parseInt(tempGoal, 10) || DEFAULT_WATER_GOAL;
    upsertMutation.mutate(
      { date: todayStr, glasses, goal: Math.max(1, newGoal) },
      { onSuccess: () => setIsGoalOpen(false) }
    );
  };
  ...
  const avgIntake = history.length > 0
    ...
    : "0";
  const daysMetGoal = history.filter(e => e.glasses >= e.goal).length;
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              <Button
                ...
                disabled={glasses === 0 || upsertMutation.isPending}
              >
          ...
          <DialogContent className="sm:max-w-sm">
            ...
            <div className="space-y-4 pt-4">
              ...
              <div className="flex gap-2">
                <Button onClick={updateGoal} className="flex-1" disabled={upsertMutation.isPending}>
          ...
          {isLoading ? (
            ...
            <div className="flex items-end justify-between gap-1 sm:gap-2 h-32">
              {history.slice(0, 7).reverse().map((entry) => {
                const dayProgress = (entry.glasses / entry.goal) * 100;
                    ...
                    <div className="w-full bg-muted rounded-t-sm relative flex-1 flex items-end min-h-[60px]">
                      <div
                        className={cn(
                          ...
                          entry.glasses >= entry.goal ? "bg-success" : "bg-primary",
        ...
        <div className="bg-card rounded-lg border border-border shadow-soft">
          ...
          {isLoading ? (
            <div className="p-12 text-center">
              ...
              <p className="text-muted-foreground">Loading water history…</p>
            </div>
          ) : isError ? (
            <div className="p-12 text-center">
              ...
              <p className="text-muted-foreground">Couldn't load your water history</p>
              ...
              {history.map((entry) => (
                <div key={entry.id} className="flex items-center gap-4 p-4 hover:bg-muted/30 transition-smooth group">
                  <div className={cn(
                    ...
                    entry.glasses >= entry.goal ? "bg-success/20" : "bg-primary/20"
                  )}>
                    <Droplets className={cn(
                      "w-5 h-5",
                      entry.glasses >= entry.goal ? "text-success" : "text-primary"
                    )} />
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-medium text-foreground">{entry.glasses} glasses</p>
                    <p className="text-xs text-muted-foreground">
                      {entry.glasses >= entry.goal ? "Goal met!" : `${entry.goal - entry.glasses} more to go`}
                    </p>
                  </div>
