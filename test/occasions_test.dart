...
void main() {
  ...
  test('settings → calendar: one yearly event, moved not duplicated',
      () async {
  ...
  test('calendar → settings: "my birthday" is understood, amma\'s is not',
      () async {
    final db = LedgerDb.forTesting(NativeDatabase.memory());
    addTearDown(db.close);
    final bridge = Occasions(db);
    ...
    expect(await settings.birthday(), isNull,
  ...
  test('next occurrence: one-offs expire, yearly rolls forward', () {
    ...
    expect(Occasions.nextOccurrence(ev('2026-08-01', EventRepeat.none), from),
        isNull);
    expect(Occasions.nextOccurrence(ev('2026-09-01', EventRepeat.none), from),
        DateTime(2026, 9, 1));
    // Birthday earlier in the year → next year's date.
    expect(Occasions.nextOccurrence(ev('2000-07-14', EventRepeat.yearly), from),
        DateTime(2027, 7, 14));
    expect(Occasions.nextOccurrence(ev('2000-12-25', EventRepeat.yearly), from),
        DateTime(2026, 12, 25));
  });
}
